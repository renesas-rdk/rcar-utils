...
- **verify vs deploy** — `/rcar-verify-build` is host-side and proves artifact
  consistency; `/rcar-deploy-image` is the on-target contract.
...
```
/rcar-customize-kernel-config → /rcar-build (kernel modules)
→ /rcar-build (fitimage image) → /rcar-verify-build
```
...
→ /rcar-verify-build → /rcar-deploy-image
  The stock and RT kernels take turns in linux-sh/, so build the one you
  want immediately before deploying it.
→ /rcar-deploy-image
```
