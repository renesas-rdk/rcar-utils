...
> [!NOTE]
> **All dependencies bundled for Windows - No Installation Required**All required tools and runtime libraries are pre-bundled in `tools/bin/windows/`:
>
...
- **fip**: TF-A FIP image name for legacy boards.
...
- **atf_fdts** *(legacy boards only)*: FCONF device tree name appended to
...
1. **Different image set** — no legacy BL2/TF-A FIP pair. Instead: **SPL** (SA0 header + SPL binary), a board-specific **U-Boot FIT** (`u-boot_sparrow-hawk.itb`), and a **PCIe PHY firmware** blob (`rcar_gen4_pcie.bin`).
2. **Board-specific FIT composition** — Yocto exports common `u-boot-nodtb-rz-cmn.bin` plus each board's U-Boot DTB. At flash time, `firmware_compile.py` runs `mkimage` to package only the selected board DTB into `u-boot_<board>.itb`. This follows the same per-board final-artifact model used by the legacy boards and avoids coupling a pre-built multi-board FIT to the U-Boot recipe.
...
| Step                              | Legacy flow (RZV2L/RZV2H/RZG2L)               | Sparrow-Hawk (R-Car V4H) flow                                             |
| --------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------ |
...
Sparrow-Hawk does not use the legacy `fiptool --tos-fw` flow that bundles
